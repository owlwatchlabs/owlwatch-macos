...
_M5.4 (cron + periodic + login/logout hooks), M5.5 (macOS app UI), and M5-close follow under this Unreleased section before the v0.6.0-m5 tag. Configuration Profiles were originally planned for M5.3 but moved out — `/var/db/ConfigurationProfiles/Store/` requires Full Disk Access (TCC) and most of its surface is already covered by what System Extensions / Login Items / Launch Services already expose. Reconsidered if a future detection rule actually needs the raw payload data._
