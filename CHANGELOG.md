...
- **M2.3: `LC_SEGMENT_64` / `LC_SEGMENT` parsing** — new `LoadCommand.segment(Segment)` variant. `Segment` carries name (e.g. `__TEXT`), VM address + size, file offset + size, `maxProtection` / `initialProtection` (as `SegmentProtection` `OptionSet` over `read` / `write` / `execute`, with `symbolicForm` giving the conventional `r-x` / `rw-` / `rwx` rendering), flags, and an array of `Section`s. `Section` carries name, parent segment name, address, size, file offset, and flags. `Slice` gains `fileOffsetInBinary: UInt64` so callers can resolve slice-relative section offsets against the whole-file bytes.
- **M2.3: `OWBinary.entropy(of:fileOffset:length:)`** — Shannon entropy (bits per byte, 0.0…8.0) over an arbitrary file range. Convenience `OWBinary.sectionEntropies(of:)` computes entropy for every non-zerofill section across every slice, returning `[SectionEntropy]` (slice index, segment name, section name, entropy, size). High entropy (>7.0) in a `__TEXT.__text` section is a strong packing / obfuscation signal.
- **M2.3: `owlwatch inspect --segments` and `--entropy`** — `--segments` lists every segment + its sections with VM protection and sizes; `--entropy` runs `sectionEntropies` and prints a per-section table with a `[HIGH]` flag on entries above 7.0 bits/byte.
